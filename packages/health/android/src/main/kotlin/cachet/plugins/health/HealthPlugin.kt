...
class HealthPlugin(private var channel: MethodChannel? = null) : MethodCallHandler, ActivityResultListener, Result, ActivityAware, FlutterPlugin {
    ...
    private var BLOOD_PRESSURE = "BLOOD_PRESSURE"
    ...
    private fun keyToHealthDataType(type: String): DataType {
        return when (type) {
            ...
            BLOOD_PRESSURE -> HealthDataTypes.TYPE_BLOOD_PRESSURE
    ...
    private fun getField(type: String): Field {
        return when (type) {
            ...
            BLOOD_PRESSURE -> HealthFields.FIELD_BLOOD_PRESSURE_DIASTOLIC
    ...
    private fun getHealthDataValue(dataPoint: DataPoint, field: Field): Any {
        ...
        return when (value.format) {
            Field.FORMAT_FLOAT -> if (!isGlucose) value.asFloat()
    ...
    private fun delete(call: MethodCall, result: Result) {
        ...
        val dataSource = DataDeleteRequest.Builder().setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS).addDataType(dataType).deleteAllSessions().build()

        val fitnessOptions = typesBuilder.build()

        try {
            val googleSignInAccount = GoogleSignIn.getAccountForExtension(context!!.applicationContext, fitnessOptions)
            Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount).deleteData(dataSource).addOnSuccessListener {
    ...
    private fun writeBloodPressure(call: MethodCall, result: Result) {
        ...
        val dataSource = DataSource.Builder().setDataType(dataType).setType(DataSource.TYPE_RAW).setDevice(Device.getLocalDevice(context!!.applicationContext)).setAppPackageName(context!!.applicationContext).build()

        val builder = DataPoint.builder(dataSource).setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS).setField(HealthFields.FIELD_BLOOD_PRESSURE_SYSTOLIC, systolic).setField(HealthFields.FIELD_BLOOD_PRESSURE_DIASTOLIC, diastolic).build()
        ...
        try {
            val googleSignInAccount = GoogleSignIn.getAccountForExtension(context!!.applicationContext, fitnessOptions)
            Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount).insertData(dataSet).addOnSuccessListener {
    ...
    private fun writeMeal(call: MethodCall, result: Result) {
        ...
        val dataBuilder = DataPoint.builder(dataSource).setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS).setField(Field.FIELD_NUTRIENTS, nutrients)

        if (name != null) {
            dataBuilder.setField(Field.FIELD_FOOD_ITEM, name as String)
        }

        dataBuilder.setField(Field.FIELD_MEAL_TYPE, MapMealTypeToType[mealType]
        ...
        try {
            ...
            Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount).insertData(dataSet).addOnSuccessListener {
                        ...
                        result.success(true)
                    }.addOnFailureListener(errHandler(result, "There was an error adding the meal data!"))
        } catch (e3: Exception) {
            result.success(false)
        }
    }
    ...
    private fun writeData(call: MethodCall, result: Result) {
        ...
        val builder = if (startTime == endTime) {
            DataPoint.builder(dataSource).setTimestamp(startTime, TimeUnit.MILLISECONDS)
        } else {
            DataPoint.builder(dataSource).setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS)
        }
        ...
        val dataPoint = if (!isIntField(dataSource, field)) {
            builder.setField(field, (if (!isGlucose) value
            else (value / MMOLL_2_MGDL).toFloat())).build()
        } else {
    ...
    private fun writeWorkoutData(call: MethodCall, result: Result) {
        ...
        val activitySegmentDataSource = DataSource.Builder().setAppPackageName(context!!.packageName).setDataType(DataType.TYPE_ACTIVITY_SEGMENT).setStreamName("FLUTTER_HEALTH - Activity").setType(DataSource.TYPE_RAW).build()
        // Create the Activity Segment
        val activityDataPoint = DataPoint.builder(activitySegmentDataSource).setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS).setActivityField(Field.FIELD_ACTIVITY, activityType).build()
        // Add DataPoint to DataSet
        val activitySegments = DataSet.builder(activitySegmentDataSource).add(activityDataPoint).build()

        // If distance is provided
        var distanceDataSet: DataSet? = null
        if (totalDistance != null) {
            // Create a data source
            val distanceDataSource = DataSource.Builder().setAppPackageName(context!!.packageName).setDataType(DataType.TYPE_DISTANCE_DELTA).setStreamName("FLUTTER_HEALTH - Distance").setType(DataSource.TYPE_RAW).build()

            val distanceDataPoint = DataPoint.builder(distanceDataSource).setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS).setField(Field.FIELD_DISTANCE, totalDistance.toFloat()).build()
            // Create a data set
            distanceDataSet = DataSet.builder(distanceDataSource).add(distanceDataPoint).build()
        }
        // If energyBurned is provided
        var energyDataSet: DataSet? = null
        if (totalEnergyBurned != null) {
            // Create a data source
            val energyDataSource = DataSource.Builder().setAppPackageName(context!!.packageName).setDataType(DataType.TYPE_CALORIES_EXPENDED).setStreamName("FLUTTER_HEALTH - Calories").setType(DataSource.TYPE_RAW).build()

            val energyDataPoint = DataPoint.builder(energyDataSource).setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS).setField(Field.FIELD_CALORIES, totalEnergyBurned.toFloat()).build()
            // Create a data set
            energyDataSet = DataSet.builder(energyDataSource).add(energyDataPoint).build()
        }

        // Finish session setup
        val session = Session.Builder().setName(activityType) // TODO: Make a sensible name / allow user to set
                // name
                .setDescription("").setIdentifier(UUID.randomUUID().toString()).setActivity(activityType).setStartTime(startTime, TimeUnit.MILLISECONDS).setEndTime(endTime, TimeUnit.MILLISECONDS).build()
        // Build a session and add the values provided
        val sessionInsertRequestBuilder = SessionInsertRequest.Builder().setSession(session).addDataSet(activitySegments)
        if (totalDistance != null) {
        ...
        val fitnessOptionsBuilder = FitnessOptions.builder().addDataType(DataType.TYPE_ACTIVITY_SEGMENT, FitnessOptions.ACCESS_WRITE)
        if (totalDistance != null) {
        ...
        try {
            ...
            Fitness.getSessionsClient(
                    ...
                    googleSignInAccount,
            ).insertSession(insertRequest).addOnSuccessListener {
    ...
    private fun getData(call: MethodCall, result: Result) {
        ...
        if (dataType == DataType.TYPE_SLEEP_SEGMENT) {
            ...
            typesBuilder.accessActivitySessions(FitnessOptions.ACCESS_READ).addDataType(DataType.TYPE_CALORIES_EXPENDED, FitnessOptions.ACCESS_READ).addDataType(DataType.TYPE_DISTANCE_DELTA, FitnessOptions.ACCESS_READ)
        }
        val fitnessOptions = typesBuilder.build()
        val googleSignInAccount = GoogleSignIn.getAccountForExtension(context!!.applicationContext, fitnessOptions)
        // Handle data types
        when (dataType) {
            DataType.TYPE_SLEEP_SEGMENT -> {
                ...
                val request = SessionReadRequest.Builder().setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS).enableServerQueries().readSessionsFromAllApps().includeSleepSessions().build()
                Fitness.getSessionsClient(context!!.applicationContext, googleSignInAccount).readSession(request).addOnSuccessListener(threadPoolExecutor!!, sleepDataHandler(type, result)).addOnFailureListener(
                        errHandler(
                                result,
            ...
            DataType.TYPE_ACTIVITY_SEGMENT -> {
                ...
                val readRequestBuilder = SessionReadRequest.Builder().setTimeInterval(startTime, endTime, TimeUnit.MILLISECONDS).enableServerQueries().readSessionsFromAllApps().includeActivitySessions().read(dataType).read(DataType.TYPE_CALORIES_EXPENDED)
                ...
                Fitness.getSessionsClient(context!!.applicationContext, googleSignInAccount).readSession(readRequest).addOnSuccessListener(threadPoolExecutor!!, workoutDataHandler(type, result)).addOnFailureListener(
                        errHandler(
                                result,
                ...
                Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount).readData(
                        DataReadRequest.Builder().read(dataType).setTimeRange(startTime, endTime, TimeUnit.MILLISECONDS).build(),
                ).addOnSuccessListener(
                        threadPoolExecutor!!,
                        dataHandler(dataType, field, includeManualEntry, result),
                ).addOnFailureListener(
                        errHandler(
                                result,
    ...
    private fun getIntervalData(call: MethodCall, result: Result) {
        ...
        Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount).readData(DataReadRequest.Builder().aggregate(dataType).bucketByTime(interval, TimeUnit.SECONDS).setTimeRange(startTime, endTime, TimeUnit.MILLISECONDS).build()).addOnSuccessListener(threadPoolExecutor!!, intervalDataHandler(dataType, field, includeManualEntry, result)).addOnFailureListener(errHandler(result, "There was an error getting the interval data!"))
    }
    ...
    private fun getAggregateData(call: MethodCall, result: Result) {
        ...
        val readWorkoutsRequest = DataReadRequest.Builder().bucketByActivitySegment(activitySegmentDuration, TimeUnit.SECONDS).setTimeRange(startTime, endTime, TimeUnit.MILLISECONDS)
        ...
        Fitness.getHistoryClient(context!!.applicationContext, googleSignInAccount).readData(readWorkoutsRequest.build()).addOnSuccessListener(threadPoolExecutor!!, aggregateDataHandler(includeManualEntry, result)).addOnFailureListener(errHandler(result, "There was an error getting the aggregate data!"))
    }

    private fun dataHandler(dataType: DataType, field: Field, includeManualEntry: Boolean, result: Result) = OnSuccessListener { response: DataReadResponse ->
    ...
    private fun sleepDataHandler(type: String, result: Result) = OnSuccessListener { response: SessionReadResponse ->
    ...
    private fun intervalDataHandler(dataType: DataType, field: Field, includeManualEntry: Boolean, result: Result) = OnSuccessListener { response: DataReadResponse ->
                ...
                for (dataPoint in dataPoints) {
                    for (field in dataPoint.dataType.fields) {
                        val healthDataItems = dataPoints.mapIndexed { _, dataPoint ->
                            return@mapIndexed hashMapOf("value" to getHealthDataValue(dataPoint, field), "date_from" to dataPoint.getStartTime(TimeUnit.MILLISECONDS), "date_to" to dataPoint.getEndTime(TimeUnit.MILLISECONDS), "source_name" to (dataPoint.originalDataSource.appPackageName
                                    ?: (dataPoint.originalDataSource.device?.model
                                            ?: "")), "source_id" to dataPoint.originalDataSource.streamIdentifier, "is_manual_entry" to dataPoint.originalDataSource.streamName.contains("user_input"))
                        }
    ...
    private fun aggregateDataHandler(includeManualEntry: Boolean, result: Result) = OnSuccessListener { response: DataReadResponse ->
        val healthData = mutableListOf<HashMap<String, Any>>()
        for (bucket in response.buckets) {
            var sourceName: Any = ""
            var sourceId: Any = ""
    ...
    private fun workoutDataHandler(type: String, result: Result) = OnSuccessListener { response: SessionReadResponse ->
        ...
        for (session in response.sessions) {
            ...
            healthData.add(
                    hashMapOf(
                            "workoutActivityType" to (workoutTypeMap.filterValues {
                                        it == session.activity
                                    }.keys.firstOrNull() ?: "OTHER"),
                            "totalEnergyBurned" to if (totalEnergyBurned == 0.0) null
                            else totalEnergyBurned,
                            "totalEnergyBurnedUnit" to "KILOCALORIE",
                            "totalDistance" to if (totalDistance == 0.0) null
                            else totalDistance,
    ...
    private fun revokePermissions(call: MethodCall, result: Result) {
        ...
        Fitness.getConfigClient(activity!!, GoogleSignIn.getLastSignedInAccount(context!!)!!).disableFit().addOnSuccessListener {
    ...
    private fun getTotalStepsInInterval(call: MethodCall, result: Result) {
        ...
        val ds = DataSource.Builder().setAppPackageName("com.google.android.gms").setDataType(stepsDataType).setType(DataSource.TYPE_DERIVED).setStreamName("estimated_steps").build()

        val duration = (end - start).toInt()

        val request = DataReadRequest.Builder().aggregate(ds).bucketByTime(duration, TimeUnit.MILLISECONDS).setTimeRange(start, end, TimeUnit.MILLISECONDS).build()

        Fitness.getHistoryClient(context, gsa).readData(request).addOnFailureListener(
                        errHandler(
                                result,
    ...
    private fun disconnect(call: MethodCall, result: Result) {
        ...
        Fitness.getConfigClient(context, googleAccount).disableFit().continueWith {
            val signinOption = GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN).requestId().requestEmail().build()
    ...
    override fun onAttachedToActivity(binding: ActivityPluginBinding) {
        ...
        healthConnectRequestPermissionsLauncher = (activity as ComponentActivity).registerForActivityResult(requestPermissionActivityContract) { granted -> onHealthConnectPermissionCallback(granted) }
    }
        ...
        scope.launch {
            MapToHCType[dataType]?.let { classType ->
                ...
                while (!pageToken.isNullOrEmpty()) {
                    request = ReadRecordsRequest(recordType = classType, timeRangeFilter = TimeRangeFilter.between(startTime, endTime), pageToken = pageToken)
                ...
                if (dataType == WORKOUT) {
                    for (rec in records) {
                        ...
                        healthConnectData.add(
                                // mapOf(
                            mapOf<String, Any?>(
                                "workoutActivityType" to (workoutTypeMapHealthConnect.filterValues {it == record.exerciseType}.keys.firstOrNull() ?: "OTHER"),
                                "totalDistance" to if (totalDistance == 0.0) null
                                else totalDistance,
                                "totalDistanceUnit" to "METER",
                                "totalEnergyBurned" to if (totalEnergyBurned == 0.0) null
                                ...
                                "totalEnergyBurnedUnit" to "KILOCALORIE",
                                "totalSteps" to if (totalSteps == 0.0) null
                                else totalSteps,
                            ...
                            if (dataType == SLEEP_SESSION) {
                                ...
                                for (recStage in rec.stages) {
                                    if (dataType == MapSleepStageToType[recStage.stage]) {
                                        healthConnectData.addAll(convertRecordStage(recStage, dataType, rec.metadata.dataOrigin.packageName))
                                    }
                                }
                            }
                        }
    ...
    fun convertRecordStage(stage: SleepSessionRecord.Stage, dataType: String, sourceName: String): List<Map<String, Any>> {
        return listOf(
            mapOf<String, Any>(
    ...
    fun getAggregateHCData(call: MethodCall, result: Result) {
        ...
        scope.launch {
            MapToHCAggregateMetric[dataType]?.let { metricClassType ->
                val request = AggregateGroupByDurationRequest(metrics = setOf(metricClassType), timeRangeFilter = TimeRangeFilter.between(startTime, endTime), timeRangeSlicer = Duration.ofSeconds(interval))
                ...
                for (durationResult in response) {
                    ...
                    val packageNames = durationResult.result.dataOrigins.joinToString { origin -> "${origin.packageName}" }

                    val data = mapOf<String, Any>("value" to (totalValue ?: 0),
                        "date_from" to durationResult.startTime.toEpochMilli(),
                        "date_to" to durationResult.endTime.toEpochMilli(),
                        "source_name" to packageNames, "source_id" to "",
    ...
    fun convertRecord(record: Any, dataType: String): List<Map<String, Any>> {
        ...
        when (record) {
            ...
            is BloodPressureRecord -> return listOf(
                    mapOf<String, Any>(
                            "value" to if (dataType == BLOOD_PRESSURE_DIASTOLIC) record.diastolic.inMillimetersOfMercury
            ...
            is RestingHeartRateRecord -> return listOf(mapOf<String, Any>(
            ...
            is BasalMetabolicRateRecord -> return listOf(mapOf<String, Any>(
                    "value" to record.basalMetabolicRate.inKilocaloriesPerDay,
                    "date_from" to record.time.toEpochMilli(),
                    "date_to" to record.time.toEpochMilli(),
                    "source_id" to "",
            ...
            is FloorsClimbedRecord -> return listOf(mapOf<String, Any>(
            ...
            is RespiratoryRateRecord -> return listOf(mapOf<String, Any>(
            ...
            is NutritionRecord -> return listOf(mapOf<String, Any>(
